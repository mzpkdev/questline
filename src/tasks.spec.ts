import { addTask, type Task, DONE_TTL_MS, remove, reorder, SEED_TASKS, toggle, visible } from "./tasks"
...
describe("tasks", () => {
    context("addTask", () => {
        it("appends a new, incomplete task with the given id", () => {
            expect(addTask([{ id: "b1", text: "one", done: false }], "b2", "two")).toEqual([
                { id: "b1", text: "one", done: false },
                { id: "b2", text: "two", done: false }
            ])
        })
        ...
        it("trims surrounding whitespace", () => {
            expect(addTask([], "b1", "  hunt the wyrm  ")).toEqual([{ id: "b1", text: "hunt the wyrm", done: false }])
        })
    ...
    context("toggle", () => {
        it("checks an open task and stamps completedAt with now", () => {
        ...
        it("re-opens a done task and clears its completedAt", () => {
    ...
    context("visible", () => {
        ...
        it("treats a completion exactly ttl old as still visible", () => {
            const edge: Task[] = [{ id: "edge", text: "edge", done: true, completedAt: NOW - DONE_TTL_MS }]
