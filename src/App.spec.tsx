...
describe("App", () => {
    ...
    context("the Rewards view", () => {
        ...
        it("redeems an affordable reward, marking it spent and draining the purse", async () => {
            ...
            // 3 - 3 leaves 0, and the reward is now a spent one-off: no redeem button, shown as redeemed.
        ...
        it("adds a custom reward to the shelf", async () => {
            ...
            fireEvent.change(await screen.findByRole("textbox", { name: "Reward name" }), {
                target: { value: "Spa afternoon" }
            })
            fireEvent.change(screen.getByLabelText("Cost in gold"), { target: { value: "15" } })
            fireEvent.click(screen.getByRole("button", { name: "Add reward" }))

            expect(await screen.findByText("Spa afternoon")).toBeInTheDocument()
        })
        ...
        it("auto-replenishes a flagged reward on redemption", async () => {
            ...
            fireEvent.click(screen.getByRole("button", { name: "Redeem Espresso" }))

            // The spent one reads redeemed; a fresh copy respawned, now locked since the purse is empty.
            await waitFor(() => expect(screen.getByText(/Redeemed/)).toBeInTheDocument())
            const copy = screen.getByRole("button", { name: "Redeem Espresso" })
            expect(copy).toBeDisabled()
            expect(copy).toHaveTextContent("Need 3 more")
        })
        ...
        it("opens the New reward card from the + tile and dismisses it on Escape", async () => {
            ...
            await waitFor(() => expect(screen.queryByTestId("add-reward-card")).toBeNull())
        })
        ...
        it("returns to the roadmap when a tab is clicked", async () => {
            ...
            fireEvent.click(screen.getByRole("button", { name: "Learn Questline" }))
            await waitFor(() => expect(nodeRoot("learn")).not.toBeNull())
            expect(screen.queryByText("Fancy coffee")).toBeNull()
        })
